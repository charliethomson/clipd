...
if %ERRORLEVEL% neq 0 (echo Download failed. & exit /b %ERRORLEVEL%)
echo Installed binary to %BINARY_DEST%

:: --- download, substitute, and register task ---
...
powershell -NoProfile -Command ^
    ...
    "Set-Content -Encoding Unicode '%TASK_XML%' -Value $xml"

schtasks /delete /tn "%TASK_NAME%" /f >nul 2>&1
schtasks /create /tn "%TASK_NAME%" /xml "%TASK_XML%"
if %ERRORLEVEL% neq 0 (echo Failed to register task. & exit /b %ERRORLEVEL%)
